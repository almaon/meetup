...
public class SetMeetingHostRoleCommandHandler implements IAsyncCommandHandler<SetMeetingHostRoleCommand> {
	...
	@Override
	public void handle(SetMeetingHostRoleCommand command) {
    	...
    	MeetingGroup meetingGroup = aggregateStore.load(meeting.getMeetingGroupId(), MeetingGroup.class);
   		...
   		meeting.setHostRole(meetingGroup, context.principalId(), new MemberId(command.getMemberId()));
